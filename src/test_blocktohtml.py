...
from blocktohtml import markdown_to_html_node, text_to_children, create_html_node, convert_bold_text, convert_italic_text, convert_link_text

class TestMarkdownToHTMLNode(unittest.TestCase):

    def test_heading(self):
        markdown = "# Heading 1"
        expected_html = HTMLNode('div', [
            HTMLNode('h1', [HTMLNode('text', ['Heading 1'])])
        ])
    ...
    def test_link(self):
        ...
        expected_html = HTMLNode('div', [
            HTMLNode('a', [HTMLNode('text', ['Example'])], props={'href': 'http://example.com'})
        ])
        result = markdown_to_html_node(markdown)
        self.assertTrue(result.equals(expected_html))
    ...
    def test_blockquote(self):
        ...
        expected_html = HTMLNode('div', [
            HTMLNode('blockquote', [HTMLNode('text', ['This is a quote.'])])
        ])
        result = markdown_to_html_node(markdown)
        self.assertTrue(result.equals(expected_html))
    ...
    def test_code_block(self):
        ...
        expected_html = HTMLNode('div', [
            HTMLNode('pre', [
                HTMLNode('code', [HTMLNode('text', ['code block'])])
            ])
        ])
        result = markdown_to_html_node(markdown)
        self.assertTrue(result.equals(expected_html))
    ...
    def test_mixed_content(self):
        ...
        expected_html = HTMLNode('div', [
            ...
            HTMLNode('ul', [
                HTMLNode('li', [HTMLNode('text', ['List item'])])
            ])
        ])
        result = markdown_to_html_node(markdown)
        self.assertTrue(result.equals(expected_html))
